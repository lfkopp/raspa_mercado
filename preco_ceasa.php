...
$get_custo  = 'SELECT data,year(data) as year1,month(data) as month1,grupo,format(avg(preco/kilos),2) as preco2 FROM `mercado` INNER JOIN `mercado_aux` ON mercado_aux.item=mercado.item WHERE  mercado="ceasa" and year(data) >="2014" and trab=1 group by data,grupo order by  data ASC';
